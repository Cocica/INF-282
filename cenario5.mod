// Quinto cenário do Trabalho 3 PO3 2020 UFV - 2020
...
dvar boolean xo[Cid][Cid];
dvar boolean xf[Cid][Cid];
dvar boolean xa[Cid][Cid];

dexpr float Cfixo = sum(j in Cid) (f[j]*y[j]);
dexpr float CFaz = sum (i in Faz,j in Cid) (0.02*c1[i][j]*x1[i][j]);
dexpr float CO = sum (j in Cid,k in Cid) (0.05*c2[j][k]*xo[j][k]*deo[k]);
dexpr float CF = sum (j in Cid,k in Cid) (0.05*c2[j][k]*xf[j][k]*df[k]);
dexpr float CA = sum (j in Cid,k in Cid) (0.05*c2[j][k]*xa[j][k]*da[k]);
...
forall (k in Cid) sum(j in Cid) xo[j][k] == 1;
forall (k in Cid) sum(j in Cid) xf[j][k] == 1;
forall (k in Cid) sum(j in Cid) xa[j][k] == 1;

forall (j in Cid,k in Cid) xo[j][k] == xf[j][k];
forall (j in Cid,k in Cid) xf[j][k] == xa[j][k];


forall (j in Cid) ( sum (i in Faz) x1[i][j] >= 40*sum(k in Cid)xo[j][k]*deo[k]+5*sum(k in Cid)xf[j][k]*df[k]+20*sum(k in Cid)xa[j][k]*da[k]);

forall (j in Cid,k in Cid)  xo[j][k] <= y[j];
forall (j in Cid,k in Cid)  xf[j][k] <= y[j];
forall (j in Cid,k in Cid)  xa[j][k] <= y[j];
...
for(i in Cid)
  for(j in Cid)
    if (xo[i][j] >= 0.001)
      writeln( Pad( nCid[i], 30 ), Pad( nCid[j], 30 ), toFixed( xo[i][j]*deo[j], 6, 0 ));
writeln("------------------------------------------------------------------")
...
for(i in Cid)
  for(j in Cid)
    if (xf[i][j] >= 0.001)
      writeln( Pad( nCid[i], 30 ), Pad( nCid[j], 30 ), toFixed( xf[i][j]*df[j], 6, 0 ));
writeln("------------------------------------------------------------------")
...
for(i in Cid)
  for(j in Cid)
    if (xa[i][j] >= 0.001)
      writeln( Pad( nCid[i], 30 ), Pad( nCid[j], 30 ), toFixed( xa[i][j]*da[j], 6, 0 ));
writeln("------------------------------------------------------------------")
